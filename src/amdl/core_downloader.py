...
# ── Monkey-patch: 绕过 gamdl 的 multiprocessing（PyInstaller 不兼容）──
def _patch_gamdl_downloader():
    """将 gamdl 的 multiprocessing 下载替换为 asyncio.to_thread 直接调用"""
    from gamdl.downloader.base import _download_ytdlp_process

    async def _fixed_download(self, stream_url: str, download_path: str) -> None:
        result_queue = queue.Queue()

        def _run():
            _download_ytdlp_process(stream_url, download_path, self.silent, result_queue)
    ...
    AppleMusicBaseDownloader._download_ytdlp_async = _fixed_download

_patch_gamdl_downloader()
...
async def _download_urls_async(
    *,
    ...
    # ── normalize path types & make absolute ──────────────
    ...
    for item in all_items:
        ...
        if not item.final_path:
            error_count += 1
            logger.error(f'Skipping item with no final_path (possibly encoding error)')
            continue
